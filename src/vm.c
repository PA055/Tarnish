...
static NativeResult timeNative(int argCount, Value* args) {
...
#define READ_LONG_CONSTANT() (frame->closure->function->chunk.constants.values[READ_BYTE() << 16 | READ_BYTE() << 8 | READ_BYTE()])
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_GET_PROPERTY: {
                ...
                runtimeError("Undefined property for '%s'.", name->chars);
                return INTERPRET_RUNTIME_ERROR;
            }
            case OP_GET_UPVALUE: {
                uint8_t slot = READ_BYTE();
